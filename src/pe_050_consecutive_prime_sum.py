...
def consecutive_prime_sum(): 
    ...
    for p in g_primes:
        ...
        for i, p2 in enumerate(primes[-2::-1]):
            ...
            if is_prime(curr_sum):
                ...
                if not existing or curr_sum_prime_count > existing and curr_sum_prime_count > max_:
    ...
    [print(p) for p in list(map(lambda p: f'{p[0]} – {p[1]} primes', sorted(dict.items(partial_sums), key=lambda item: item[0] / item[1])))[0:10]]
